...
Each request opens its own TLS connection (see [Protocol limits](#protocol-limits)), so it has a real cost, and the TV's own JSON-RPC server only handles one connection at a time — concurrent requests get the connection reset outright, confirmed directly against the TV. A fast slider drag in Apple Home can still fire many brightness writes in a couple of seconds, faster than the TV can serially answer them, so `async_queue_backlight` keeps only the most recently requested target whenever a write is already in flight: that write finishes normally, and the writer then immediately sends whatever the latest target has become, dropping anything superseded in between while it was busy. It is intentionally fire-and-forget from the entity's perspective — `async_turn_on`/`async_turn_off` publish the optimistic target immediately and return, and a write failure is logged rather than raised, since by the time a queued write finishes there is often already a newer target superseding it.

Only the first target of a burst gets the neighbor-value nudge described below; every later one in the same burst is a single plain write, because it already differs from whatever was just written, which is what makes a write land instantly in the first place. In practice this means an active slider drag sends every position it actually passes through (network speed permitting) rather than jumping straight to wherever it ends, matching the sibling `tvolve` iOS project's own behavior.

#### The neighbor-value nudge

A bare `backlightControl` write is accepted by a Samsung QN90B, but the physical picture then fades to it over roughly 10-15 seconds instead of snapping there. A confirmatory read of the same value shortly after (what the sibling `tvolve` project does after every write) was tried first and made no difference. Controlled testing directly against the TV (bypassing this integration and `tvolve` both) isolated the actual trigger: writing a *different* value immediately before the real target makes the panel jump straight to the target instead of fading in; writing the same value twice does not. `async_set_backlight` therefore always writes a throwaway neighboring value first (the target plus or minus one, whichever stays in range), waits 150ms, then writes the real target. This also explains why dragging a slider always looked instantly responsive: every intermediate position sent while dragging differs from the last, so each one already re-triggers the jump on its own; only a single, one-shot write exposed the fade.
