...
A bare `backlightControl` write alone was empirically accepted by a Samsung QN90B but left visibly unapplied on the physical picture for a long time — in practice, until whatever the next unrelated request to the TV happened to be, which for this integration meant waiting on the next scheduled coordinator poll, up to ten seconds later. A sibling project (`tvolve`) already follows every write with a short pause and a plain read of the same method; empirically, that is what makes the picture react immediately instead of waiting on some later, unrelated request to nudge it. `async_set_backlight` does the same: write, sleep 150ms, read. The read's result is not used for anything and its failure does not fail the write, since only its side effect on the TV matters here.
